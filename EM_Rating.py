...
steam_clean = pd.read_csv(locationUsersFile, usecols=[0, 1, 2, 3, 4])

game_freq = steam_clean.groupby(by='game_name').agg({'user_id': 'count', 'hours': 'sum'}).reset_index()
...
users = pd.DataFrame({'user_id': sorted(steam_clean_pos['user_id'].unique()), 'user_id': range(len(steam_clean_pos['user_id'].unique()))})
...
def game_hrs_density_p(GAME, nclass, print_vals=True):
...
# Building data frame for plotting
  ...
  game_plt = ggplot(aes(x='game_data', y='stat(density)'), game_data) + geom_histogram(bins=45, colour = "black", alpha = 0.7, size = 0.1) + \
                ...
                ggtitle(t_GAME)
  return game_plt
a = game_hrs_density_p(pred, "Fallout4", 5,False)
print(a)
